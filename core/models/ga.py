...
class GAEncoder(nn.Module):
    def __init__(self, ipa_conf):
        ...
        self.current_seq_embedder = nn.Linear(20, self._ipa_conf.c_s)
        ...
        # )
        self.res_feat_mixer = nn.Sequential(
            nn.Linear(3 * self._ipa_conf.c_s + self.angles_embedder.get_out_dim(in_dim=15), self._ipa_conf.c_s),
            nn.ReLU(),
            nn.Linear(self._ipa_conf.c_s, self._ipa_conf.c_s),
        )
    ...
    def forward(self, t, rotmats_t, trans_t, angles_t_mu, angles_t_pi, seqs_t, node_embed, edge_embed, generate_mask, res_mask):
        num_batch, num_res, _ = seqs_t.shape
        angles_t = angles_t_mu*angles_t_pi
        
        seqs_t = seqs_t * 2 - 1     # from 1/K \in [0,1] to 2/K-1 \in [-1,1]
